...
Key egress (formerly check (e)) now lives in ``tools/check_key_egress.py``, which
drives Semgrep rules instead of a hand-rolled AST walk. Check (e) was removed in
round 6 rather than kept alongside: a round-5 adversarial reviewer EXECUTED a
key-bearing, redirect-following exfil sender that it passed with exit 0, and two
overlapping gates where the weaker one is the one people read is worse than one
gate that holds. See docs/adr/0019.
