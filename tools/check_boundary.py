...
or charon.* packages — no third-party dependencies. Relative imports inside
engine/ are intra-charon and are always allowed. This pass is a no-op while
...
def _engine_allowed(name: str | None) -> bool:
    """Return True iff the absolute import is permitted inside engine/ (stdlib or charon.*)."""
    if not name:
        return True
    head = name.split(".")[0]
...
def scan_engine_file(path: Path) -> list[str]:
    ...
    Relative imports (level >= 1) are intra-charon and are always permitted.
    """
    violations: list[str] = []
    tree = ast.parse(path.read_text(), filename=str(path))
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            # Relative imports (level >= 1) are intra-charon — always allowed.
            if node.level >= 1:
                continue
