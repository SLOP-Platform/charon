"""WIRE-TOOL-REPAIR — tool_repair.py was a written+tested but DEAD safety net
(never wired into the forwarder). CG-critical: tool-calling is fragile/fails for
several model pools; a malformed ``tool_calls[].function.arguments`` string
(single-quoted keys/values, trailing commas — common local/small-model output)
must be repaired on the REAL served-response path, not a parallel helper.
...
def _drive(tmp_path: Path, *, wire_tool_repair: bool) -> dict:
    ...
    if wire_tool_repair:
        kwargs["tool_repair"] = ToolCallRepair()
