...
| **Featherless** | `https://api.featherless.ai/v1` | ✅ | ✅ | **No free tier.** $10 Basic (≤15B, 2 conc) / $25 Premium (any model, 4 conc), flat unlimited tokens — **⚠ 32K CONTEXT/SESSION CAP** (operator 2026-07-10): 4 tiny OR 1 bigger session; heavy work must be **STAGED into ≤32K chunks sequentially** → unfit for large-context builds | **No — does not log prompts/completions** | Resale unverified verbatim (flat‑rate seat → check AUP) |
| **Synthetic.new** ⛔DROPPED 2026-07-10 | `https://api.synthetic.new/openai/v1` | ⚠️ advertised, under‑documented | ✅ | $30/mo pack: **500 req/5h**, **$24/wk** credit ceiling, **1 concurrent/model** (this kills gateway fan-out → DROPPED; use OpenRouter for GLM-5.2) | **No — no train, no retention** | **No** (§2.2 account‑sharing + §4(vi) substitute‑service) |
...
### 2.5 Featherless — **no free tier; $10/$25 flat unlimited tokens BUT 32K context/session cap**
- **⚠ RECURRING CORRECTION (operator, 2026-07-10) — stop re-recommending this for heavy builds:** the $25 Premium plan is capped at **~32K context per session**. The "4 concurrent" budget means **4 really small sessions OR 1 bigger one** — and because of the 32K ceiling, any heavy/long-context coding build must be **STAGED into ≤32K-context sessions run one after another**, not done in a single big session. → **Featherless is UNFIT as a drop-in for large-context builds; use only for small or explicitly-staged work.** (Prior doc revisions kept omitting this and re-recommending it for heavy builds — do not.)
- **No free tier.** $10 Basic (≤15B params, 2 concurrent) / $25 Premium (any model, 4 concurrent) / Agent $100–$200. Flat‑rate **unlimited tokens** (throttled by **concurrent slots** not RPM/TPM) **within the 32K/session context cap above**. Source: https://featherless.ai/docs/plans
...
- **ToS:** *"Featherless does not log chats, prompts, or completions sent through our API… processed in real time and not stored."* Source: https://featherless.ai/docs/privacy-and-logging. Proxy/resale clause not verified verbatim (check https://featherless.ai/terms).
  - **Verdict:** strongest privacy story; good flat-cost floor **only for small/staged (≤32K-context) work** — NOT for large-context builds (see the 32K cap above).
