...
#   (1) CANARY FIRES  — the wrapper flags fleet/tests/fixtures/bandit-known-bad.py (>=1 finding, exit 1).
#   (2) FAIL-ON-REVERT— point the SAME wrapper at a neutered config (BANDIT_CONFIG with skips: [B602,...])
#                        -> findings drop to 0 and the wrapper goes GREEN. This proves the RED in (1) is
#                        produced BY bandit's B602 (subprocess shell=True) test, not by accident. If
#                        neutering did NOT flip it, this step FAILS — reverting the test breaks the
#                        canary, as required. (Severity CANNOT neuter it: B602 is HIGH, so even
#                        BANDIT_SEVERITY=high still catches it — the gate is severity-robust.)
...
#                        "no issues" over an empty dir (verified 1.9.4). The wrapper's OWN files-scanned
#                        guard reds on that (exit 2) — VERSION-INDEPENDENT, not relying on bandit's exit
#                        code. This is the bandit analog of semgrep/gitleaks' zero-rule vacuous guard:
#                        for bandit the false-green is a scanned-nothing scope, not a no-rule config.
#
# Fully hermetic: neutered config + empty scope live in mktemp dirs; nothing tracked is mutated. Run:
#   bash fleet/tests/bandit-canary.test.sh   (exit 0 = all pass)
set -uo pipefail
FLEET="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WRAP="$FLEET/checks/bandit.sh"
FIXTURE="$FLEET/tests/fixtures/bandit-known-bad.py"
PASS=0; FAIL=0
...
# A neutered config: skip the B602 (and its low-severity B404 import companion) tests -> the fixture's
# shell=True finding disappears. This is bandit's native --configfile knob (what the wrapper exposes
# via BANDIT_CONFIG), the faithful analog of neutering semgrep's flagship rule / gitleaks' ruleset.
...
# wrapper's default (medium) severity threshold. bandit exits 1 when it finds issues, so capture its
# output FIRST (|| true) and parse separately — otherwise pipefail would let bandit's exit-1 trip the
# fallback and corrupt the count.
count(){ local tgt="$1"; shift
  local rep; rep="$(bandit -q -f json --severity-level medium "$@" -- "$tgt" 2>/dev/null || true)"
...
# (3) CLEAN PASSES: an ordinary snippet with no insecure pattern (no shell=True, no subprocess import).
...
# (4) VACUOUS GUARD (VERSION-INDEPENDENT): a scan that touches ZERO files can never find anything, so
#     its green is a false receipt. bandit ITSELF exits 0 "no issues" over an empty dir — so we assert
#     the WRAPPER's own files-scanned guard reds it (exit 2), NOT bandit's exit code. This is the
#     bandit analog of the semgrep/gitleaks zero-rule vacuous guard (scope-to-nothing == vacuous here).
