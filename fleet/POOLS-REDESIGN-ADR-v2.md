...
**Operator requirement (2026-07-08) — quality-equivalent, drain-ordered tier redundancy:**

*(This SUPERSEDES and refines the earlier "multiple tested members per tier" one-liner: the members must not merely be tested — they must be a cluster of ~2–4 that hold up as quality-equivalent on the operator's real code, drain-ordered across independent quotas.)*

- **The rule:** Each tier MUST carry **~2–4 QUALITY-EQUIVALENT models, each on an INDEPENDENT provider/quota, drain-ordered** — so that when one leg runs out of usage, routing silently falls through to the next EQUIVALENT model with **NO work stoppage and NO quality drop**. This is the no-single-point-of-failure guarantee made concrete: not just "more than one member," but *enough interchangeable members that draining any one is invisible to the operator*.
- **What "equivalent" means (load-bearing):** equivalent **FOR THE OPERATOR'S REAL CODE WORK**, established by **real-code testing + the real-outcomes benchmark** (BENCH-REGROUND-LIVE / #26), **NOT** "both are good models" or a public leaderboard. That *tested* equivalence is precisely what makes a mid-drain switch seamless — the routing can swap legs because the two produce interchangeable results on the operator's actual tickets.
- **What the grades job must therefore deliver:** find **CLUSTERS of ~2–4 quality-equivalent models per tier**, not just rank a single #1. The benchmark's output for this requirement is an equivalence-cluster per tier, drain-ordered, superseding the "just pick the top model" reading. (Consistent with the "seeded then refined, never gated on benchmark" tier-assignment stance above — seeding gives provisional cluster members; the real-outcomes signal confirms/prunes them.)
- **Independent quotas:** the ~2–4 legs MUST sit on **distinct providers/accounts/quotas** so they drain independently. Two aliases of the same upstream balance are ONE leg, not two (cf. the usage-profile finding that nearly the whole catalog shares the identical `[nanogpt, openrouter]` chain → one balance event dries everything). Widening with non-balance-gated members is the lever.
- **How the pipeline delivers it (buildable mechanics):**
  1. **METER-MODEL-PROVIDER** — per-model × per-provider usage/spend accounting detects a *draining leg* (approaching quota/balance/rate cap) before it hard-fails.
  2. **FREE-TIER-QUOTA-SPILL / PFF-P2 cross-model substitution** — on a draining/exhausted leg, route to the **next equivalent model** in the tier's drain-ordered cluster (not just the next *provider* of the same model — cross-**model** substitution within the equivalence cluster).
  3. **Transparent to the agent** — the calling agent keeps hitting the same tier/pool id; the substitution happens under the alias, so the agent sees no interruption.
- **SMART switch-timing requirement (operator, 2026-07-08) — predict-and-ride, never run dry mid-task:** the quota-exhaustion switch MUST be smart about *WHEN* to switch, not merely reactive to a hard failure. Net rule: **switch at the least-disruptive boundary that still avoids running dry**, driven by usage-remaining + task/session boundaries.
  - **(a) Prefer TASK boundaries.** Switching is cleanest at **task boundaries**, not mid-task; a mid-session swap carries context but can shift *style* even between equivalents. Tighter tested equivalence → more seamless swap; treat residual style drift as a signal the equivalence cluster needs tightening.
  - **(b) PREDICT, don't wait for the 402.** Use the METER's usage / quota-remaining signal to **forecast** exhaustion ahead of time — never wait for a hard 402/quota-wall to trigger the switch.
  - **(c) RIDE IT OUT when there's headroom.** If a leg is running low but has **enough headroom to finish the current task (or session)**, keep using it and switch at the **least-disruptive boundary** — task end, or even **session end if usage is low-but-sufficient**. Don't switch early just because a leg is "getting low."
  - **(d) But NEVER run dry mid-task.** If predicted usage will NOT finish the current task, switch at the **task START / last safe boundary** instead — proactively, before the leg dies mid-work.
  - **Signal + actuator:** the usage/quota-remaining forecast comes from **METER-MODEL-PROVIDER**; the actual leg-swap is executed by **FREE-TIER-QUOTA-SPILL / PFF-P2** cross-model substitution. The meter is the predictor; the spill is the actuator.
- **Testing SCOPE implication:** the benchmark/real-code trial must evaluate **enough candidates per tier to find 2–4 that hold up on real code** — not stop at the first passing model. Sizing the trial pool is a direct consequence of this requirement.
- **Cross-refs:** METER-MODEL-PROVIDER (draining-leg detection) · FREE-TIER-QUOTA-SPILL / PFF-P2 (cross-model substitution) · BENCH-REGROUND-LIVE / #26 + #25 (real-outcomes equivalence signal) · DRAIN-ROUTING / COST-RANK-AUTO (balance-aware demotion the drain-ordering rides on).
