...
difficulty: 3
work_class: refactor
branch: feat/wci-dec-src-charon-providers-py
owns: src/charon/providers.py
depends_on: 
auto_generated: wci-contention
wci_contention_target: src/charon/providers.py
...
source: |
  AUTO-GENERATED by fleet/wci-contention.sh --generate: src/charon/providers.py is owned by 4 board
...
accept: |
  - src/charon/providers.py is split along its seams; each resulting module is owned by AT MOST one
...
scope: |
  ONLY the decomposition of src/charon/providers.py and the mechanical re-pointing of its callers.
