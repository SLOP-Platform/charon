...
difficulty: 3
work_class: refactor
branch: feat/wci-dec-src-charon-config-py
owns: src/charon/config.py
depends_on: 
auto_generated: wci-contention
wci_contention_target: src/charon/config.py
...
source: |
  AUTO-GENERATED by fleet/wci-contention.sh --generate: src/charon/config.py is owned by 6 board
...
note: |
  Owners at generation time: (none live).
...
accept: |
  - src/charon/config.py is split along its seams; each resulting module is owned by AT MOST one
...
scope: |
  ONLY the decomposition of src/charon/config.py and the mechanical re-pointing of its callers.
  Does NOT change behaviour, and does NOT re-slice the owning tickets' own work.
ds: |
  ## Dependencies & sequence
  depends_on: (none — no live owner)
