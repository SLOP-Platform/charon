...
difficulty: 3
work_class: refactor
branch: feat/wci-dec-fleet-review-pool-sh
owns: fleet/review-pool.sh
depends_on:
dep-kind: build
parked: true
auto_generated: wci-contention
wci_contention_target: fleet/review-pool.sh
...
source: |
  AUTO-GENERATED by fleet/wci-contention.sh --generate: fleet/review-pool.sh is owned by 4 board
...
note: |
  Owners at generation time: MODEL-HARDCODE-PURGE, PR-QUEUE-REST-ETAG, REVIEWER-TAB-POOL, SANDBOX-CONTAINMENT.
...
accept: |
  - fleet/review-pool.sh is split along its seams; each resulting module is owned by AT MOST one
...
scope: |
  ONLY the decomposition of fleet/review-pool.sh and the mechanical re-pointing of its callers.
  Does NOT change behaviour, and does NOT re-slice the owning tickets' own work.
ds: |
  ## Dependencies & sequence
  depends_on: MODEL-HARDCODE-PURGE, PR-QUEUE-REST-ETAG, REVIEWER-TAB-POOL, SANDBOX-CONTAINMENT
...
stale: |
  PARKED 2026-08-01 by fleet/wci-contention.sh --generate: owner MODEL-HARDCODE-PURGE carries a justified 'serial_justified:' — the file is serial BY DESIGN.
