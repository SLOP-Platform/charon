...
source: |
  AUTO-GENERATED by fleet/wci-contention.sh --generate: src/charon/proxy_server.py is owned by 10 board
...
note: |
  Owners at generation time: ORDER-A-COST-PRIMARY-LAND.
...
accept: |
  - src/charon/proxy_server.py is split along its seams; each resulting module is owned by AT MOST one
...
scope: |
  ONLY the decomposition of src/charon/proxy_server.py and the mechanical re-pointing of its callers.
