...
- **land.sh MODEL:** land.sh snapshots the MAIN CHECKOUT's CURRENT HEAD onto `<branch>`. To land an EXISTING branch (a droid's submitted branch, a recovered/edited one), FIRST `git checkout -B <branch> <sha>` so HEAD is that branch, OR use `land-push.sh <branch> <repo>` (by-name). ✅ NOW ENFORCED: land.sh has an EXISTING-BRANCH GUARD that REFUSES + prints the fix when `<branch>` has commits not in HEAD (so it can't silently clobber anymore). Also: land.sh still REFUSES a dirty tree with no owns-scope → set stray untracked files aside first.
