...
def main():
    ...
    if not new_tests:
        return common.emit(40, "pass" if scope else "fail", "bug found & fixed, suite green, but no discriminating test added")

    buggy_content = (baseline / "gateway/normalize.py").read_text()
    passed_buggy, _, _ = common.swap_and_run_pytest(worktree, "gateway/normalize.py", buggy_content, test_args=new_tests)
    discriminates = not passed_buggy

    if not discriminates:
        return common.emit(20, "pass" if scope else "fail",
                            "test present but does not fail against the injected/buggy code (never located the bug)")

    isolates = isolates_namespaced_id(worktree, baseline, new_tests)
    if not isolates:
        return common.emit(70, "pass" if scope else "fail",
                            "correct fix + green suite, but test doesn't isolate the namespaced-id minority path")

    if not scope:
        return common.emit(75, "pass", f"correct + discriminating + isolating but diff touched files beyond normalize.py/tests/: {changed}")

    return common.emit(100, "pass", "subtle bug found, isolating test discriminates buggy/fixed, suite green, scope clean")
