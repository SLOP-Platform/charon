...
def yaml_ok(worktree):
    ...
    if shutil.which("actionlint"):
        proc = subprocess.run(["actionlint", str(path)], capture_output=True, text=True, cwd=str(worktree))
...
def hardcoded_port_ok(worktree):
    ...
    if re.search(r"HOST_PORT[:=][-\s]*[\"']?\d{4,5}\b", smoke_text) or re.search(r"HOST_PORT:-\s*\d{4,5}", smoke_text):
...
def behavioral_masking_fixed(worktree):
    ...
    if rc_unreachable == 0:
        return False, "smoke.sh exits 0 against an unreachable health endpoint (masking bug still present)"
    if rc_healthy != 0:
        return False, "smoke.sh exits non-zero even against a genuinely healthy stub (over-broken, not just fixed)"
...
def main():
    ...
    reason = f"yaml={ok_yaml}({msg_yaml}); pipefail_static={ok_pipefail_static}; behavioral={ok_behavioral}({msg_beh}); port={ok_port}({msg_port}); markers={msg_markers}"

    if not ok_markers:
        score = 25 if defects_fixed > 0 else 0
        return common.emit(score, "fail", "gate weakened: an existing CHECK assertion was removed - " + reason)
