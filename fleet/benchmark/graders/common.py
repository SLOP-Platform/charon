...
def file_diff(baseline: Path, worktree: Path, relpath: str) -> str:
    ...
    return "".join(difflib.unified_diff(b_lines, w_lines, fromfile=f"a/{relpath}", tofile=f"b/{relpath}"))
...
def swap_and_run_pytest(worktree: Path, swap_relpath: str, swap_content: str, test_args=None, timeout=60):
