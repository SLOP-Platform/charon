...
FUNCTIONAL_SNIPPET = (
    ...
    "providers = [Provider('a','u','cheap',5), Provider('b','u','cheap',1), Provider('c','u','cheap',9)]; "
...
def main():
    ...
    passed_buggy, _, _ = common.swap_and_run_pytest(worktree, "gateway/providers.py", buggy_content, test_args=new_tests)

    discriminates = passed_fixed and not passed_buggy

    if discriminates and scope:
        return common.emit(100, "pass", "fix correct, new test fails on buggy code / passes on fixed, scope clean")
    if not scope:
        return common.emit(70 if discriminates else 40, "pass",
                            f"correct fix{' + discriminating test' if discriminates else ''} but diff touched files beyond providers.py/tests/: {changed}")
    return common.emit(70, "pass", "fix correct and scoped, but new test does not distinguish buggy vs fixed code")
