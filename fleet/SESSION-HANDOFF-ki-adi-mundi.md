# Charon Fleet — Session Handoff (2026-07-21T06:59:58Z) — ki-adi-mundi
...
```
Read and fully follow /home/stack/charon-private/fleet/SESSION-HANDOFF-ki-adi-mundi.md — you are the fresh Charon fleet MANAGER, carry it out, then flip to fleet mode.
```
...
**Session:** ki-adi-mundi
**Generated:** 2026-07-21T06:59:58Z
**Product HEAD:** 7e16e4a — current with origin/master
**Rig HEAD:** 5b9d43c — current with origin/master

---
<!-- GENERATED-STATE v1 (do not hand-edit) generated=2026-07-21T07:00:00Z -->
...
origin-master product = 7e16e4a76850e8f576bb38dfd0498f52b602134a  # SLOP-Platform/charon
origin-master rig = 5b9d43cfbad0dd1a831ccd1fe2da6d07818a451e  # Nnyan/charon-private
...
- rig /home/stack/charon-private [master]: 0 commit(s) ahead of origin/master, 2 file(s) uncommitted
...
- rig /home/stack/charon-private-wt/KIT-FISTO [handoff/kit-fisto]: no-upstream commit(s) ahead of <none>, 2 file(s) uncommitted
...
## Direction (anchored — MUST NOT drift)

This session LOCKED the anti-drift doctrine the operator has fought for across sessions.

**TOP directive (operator, verbatim — now `MANAGER-OPERATING-RULES.md` §0):** no rig-as-product; ADOPT-first; hand-rolling is the LAST, ugly choice, met ADVERSARIALLY with significant NEGATIVE weight; the stdlib-only prohibition is REMOVED; refocus on the shippable **gateway product**.

Plus, now also in §0: **e2e + dogfood are the NORM for all key money-path code** (gateway request/routing/failover/cost/keys/provider-send) — never unit-tests-only; a full-path e2e test + a real dogfood run must exist and pass before money code lands.

The recurring failure was operator anti-hand-roll decisions being dropped/inverted across handoffs (`HANDOFF-FAILURE-ANALYSIS` mode-4). They are now in the LOADED rules (§0) **and** in memory so they cannot drift. Any session proposing to hand-roll / build rig MUST argue against it and consult the **HAND-ROLL JUSTIFICATION ANTI-PATTERNS (AP-1..AP-11)** in `fleet/state/EVAL-REGISTRY.md`.
...
```
rig master HEAD:    5b9d43c
rig master subject: chore(board): retire hand-rolled gateway tickets; un-park litellm.Router MVP
product master HEAD:    7e16e4a
product master subject: feat(gateway): adopt litellm.Router (library) for the commodity plane — first slice

--- last 5 rig master commits ---
5b9d43c chore(board): retire hand-rolled gateway tickets; un-park litellm.Router MVP
f7acecf doctrine(gateway-mvp): flip LiteLLM-Router to ADOPT + sequenced MVP tickets
db4b72c doctrine(eval): add HAND-ROLL JUSTIFICATION ANTI-PATTERNS (negative column)
3e884c6 doctrine(adopt-first): operator HARD STANDING DIRECTIVE §0 — adopt-first, hand-roll is last-resort with negative weight, stdlib-only prohibition removed
64cee96 chore(fleet): retire inert mislabeled hand-rolled memory code (FN1/FN2/FN3)
...
**This session's landed chain (do NOT regress):**

Rig master (chain):
- §0 ADOPT-FIRST directive — `3e884c6`
- hand-roll anti-patterns negative column — `db4b72c`
- EVAL-REGISTRY rows + 5 KS31 tool tickets (SEMGREP / SHELLCHECK-BLOCKING / GITLEAKS / BANDIT / VULTURE) — `5f196a6`
- memory-retire FN1/FN2/FN3 (inert hand-rolled memory removed) — `64cee96`
- gateway-MVP tickets + LiteLLM-router ADOPT row — `f7acecf`
- retire superseded hand-rolled-gateway tickets — `5b9d43c`

Product master (chain):
- egress Phase-1 app-layer key-exfil control — `d944a26`
- stdlib-only prohibition removed — `56a00a1`
- gateway `litellm.Router` adopt FIRST SLICE (opt-in, live path unchanged) — `7e16e4a`

---

## Carry-forward (every item gets a verdict)

- **luminara DONE-CONTRACT (two adopt-decisions) → DELIVERED.**
  - **SECURITY** = Smokescreen egress + nginx credproxy custody (A+C, Phase-1-first): Phase-1 app-layer **LANDED** (`egress.py`, `d944a26`). **Phase-2 (Smokescreen + nginx custody, live 4-LOM deploy) DEFERRED** — this is the REAL enforcing control; master still carries the key-exfil class at the network layer until Phase-2 lands.
  - **META-GATE** = a combination (Semgrep CI required-check + reuse in-tree `ReviewerCircuitBreaker` + mechanized adversarial verification); the in-session doom-loop / hook glue was **CUT per §0** (rig-gold-plating).
- **basic-memory MCP → ADOPTED + LIVE** (indexed 146 notes; `claude mcp list` shows Connected). Manager memory recall now via basic-memory. The hand-rolled `fleet/memory/` (FN1/2/3) was retired.
- **FN2 ledger-decay intent** → preserved as ROUTER-LEDGER-DECAY ticket.
- **SLOP propagation** → SLOP is UPSTREAM of the adopt doctrine (already has rule a' / `EXTERNAL-TOOLS.md`); 2 P1 tickets created (#1342 shellcheck fail-open harden, #1343 egress-approach eval). basic-memory: SLOP **DTC-REJECTED** it (ADR-0023, "no token win") — Charon adopted it anyway for on-demand recall; operator aware.
...
### Manager's first actions (priority order — product-first per §0)

1. **GATEWAY-LITELLM-ADOPT live wire-in slice (money-code).** Replace the live `forward_with_failover` / stdlib `http.server` money-path with `litellm.Router`; re-host the KEEP policy as Router hooks (silent-downgrade SR-1/2, drain-then-park, funding-class, streaming-downgrade); wire `no_redirect_client` into `make_router`; add an explicit key handler to close the allowlisted-cross-provider edge (review note). **e2e + dogfood REQUIRED.** Promote `litellm` from the optional `router` extra to core deps when wired live.
2. **GATEWAY-GRADE-ORDER-MVP.** Neutral PRODUCT-side grade store/format (NOT the rig scorecard) + grade-ordering overlay into `litellm.Router`, keyed by taxonomy work-kind, fail-open, byte-identical cold-start. Acceptance: fail-on-revert (grade-A 2nd-cheapest beats grade-F cheapest).
3. **Phase-2 security** (Smokescreen egress + docker denial + nginx credproxy) — live 4-LOM deploy, operator-gated.

---

## Operator-pending (exact — action required)

- **Decide product land flow:** route product lands through real PRs so CI `gate` runs, OR keep the manager bypass. Product master took **3 bypass lands** this session (`d944a26` / `56a00a1` / `7e16e4a`) — branch protection ("Required status check gate") was bypassed by the manager token each time; **CI never ran on master.**
- **`CI_RUNNER` still UNSET** (charon-ci 5-runner pool idle) — Semgrep / CodeQL / Layer-D required-checks are **INERT** until `CI_RUNNER=["self-hosted","charon-ci"]` is set.
- **Memory-index near load cap** (~22.7KB / 24.4KB); basic-memory adopted for pull-recall; the light index-trim (keep behavioral rules in index, pull facts via basic-memory) is DEFERRED per operator — do it before the index hits the cap or `MEMORY.md` stops loading.
...
- Manager CANNOT `gh pr merge` or raw `git push` (deny-list) — land ONLY via `land-push.sh`. **land-push to PRODUCT master BYPASSES branch protection** (the token has bypass) — CI does NOT run; use real PRs when you want CI.
- **Base-integrity: sync local main to origin AFTER every land** — this session a build forked off a stale local main (missing `egress.py`) because lands happened from worktrees and local main wasn't pulled. Run `git -C <repo> pull --ff-only` after each land.
- **Worktree gate FALSE-REDs** on missing gitignored `state/done/` markers — `cp -rn /home/stack/charon-private/fleet/state/done/. <wt>/fleet/state/done/` before gating a rig branch in a fresh worktree.
- **Tool/subagent SUCCESS lines LIE** — verify outcomes (`ls-remote`, `gh --json state`, present-on-master), never the success line.
...
```
/home/stack/code/charon                                                                         7e16e4a [master]
...
/home/stack/charon-private                                                                       5b9d43c [master]
...
/home/stack/charon-private-wt/KIT-FISTO                                                          5b9d43c [handoff/kit-fisto]
...
2026-07-21T06:56:44Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T06:56:44Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T06:56:44Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T06:56:44Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T06:56:44Z	out	my-model	too-slow-failover	rc=124; budget=1800s; model streamed output but did not finish -- latency-is-a-failure-class, model-attributable
2026-07-21T06:56:45Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T06:56:45Z	out	my-model	infra-fault-failover	rc=3; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T06:56:45Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T06:56:45Z	out	my-model	error-failover	rc=1; non-limit, non-infra failure (genuine model-attributable result)
2026-07-21T06:56:45Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
```
### Git
```
...
```
  ...
  CHARON-FLEET STATUS @ 2026-07-21T07:00:29Z
  ...
  ADR0016-DEPLOY-PRICED-COMPLETENESS strong  PR-OPEN   feat/adr0016-priced-completeness-guard 124h10m ago
  API-DECOMPOSE-CYCLE-FIX economy PR-OPEN   feat/api-decompose-cycle-fix 118h40m ago
  ASSIGN-DISPATCH-PICK-FIX strong  PR-OPEN   feat/assign-dispatch-pick-fix 120h52m ago
  BANDIT-ADOPT strong  blocked   feat/bandit-adopt      needs SEMGREP-CI-REQUIRED-CHECK
  BENCH-OOB-GRADING frontier PARKED    feat/bench-oob-grading unclaimable — see note:
  BENCH-PROVISIONAL-SCORING strong  PR-OPEN   feat/bench-provisional-scoring 120h33m ago
  CAPABILITY-ACTUALS-DEADREF-CLEANUP economy PR-OPEN   feat/capability-actuals-deadref-cleanup 120h25m ago
  CAPTURE-WIRING-TIMEOUT-FIX strong  PR-OPEN   feat/capture-wiring-timeout-fix 51h37m ago
  CG-LAN-OPEN-UI strong  PARKED    feat/cg-lan-open-ui    unclaimable — see note:
  CHARON-FLOWCHART strong  PR-OPEN   docs/charon-flowchart  119h33m ago
  ...
  DECOMPOSER-ROUTE-THROUGH-SWITCHBOARD strong  PR-OPEN   feat/decomposer-route-through-switchboard 120h41m ago
  DONE-SH-INTEGRITY-FIX strong  blocked   feat/done-sh-integrity-fix needs GITHUB-LIMITS-HARDENING, VERIFY-MERGED-REPO-AWARE
  DROID-LIFECYCLE-REAP strong  PR-OPEN   feat/droid-lifecycle-reap 121h24m ago
  ...
  FOREMAN-MULTI-TRIGGER economy PR-OPEN   feat/foreman-multi-trigger 120h46m ago
  FOREMAN-WIRE economy DONE      feat/foreman-wire      -
  FT-CATALOG-SEED economy PR-OPEN   feat/ft-catalog-seed   145h36m ago
  FT-LIMITS-GROQ-RECONCILE economy PR-OPEN   feat/ft-limits-groq-reconcile 53h09m ago
  FT-WIRE-QUOTA strong  PARKED    feat/ft-wire-quota     unclaimable — see note:
  GATEWAY-GRADE-ORDER-MVP strong  blocked   feat/gateway-grade-order-mvp needs GATEWAY-LITELLM-ADOPT
  GATEWAY-LITELLM-ADOPT strong  ready     feat/gateway-litellm-adopt -
  GATEWAY-NONTOKEN-METERING strong  PR-OPEN   feat/gateway-nontoken-metering 123h28m ago
  ...
  GITHUB-LIMITS-HARDENING strong  PR-OPEN   feat/github-limits-hardening 121h29m ago
  GITLEAKS-ADOPT strong  blocked   feat/gitleaks-adopt    needs SEMGREP-CI-REQUIRED-CHECK
  GRACEFUL-DEGRADE frontier PR-OPEN   feat/graceful-degrade  112h21m ago
  ...
  HANDOFF-ROOT-ARCHIVE economy PR-OPEN   feat/handoff-root-archive 107h03m ago
  INERT-INSTANCE-DETECT strong  blocked   feat/inert-instance-detect needs CAPABILITY-ACTUALS-DEADREF-CLEANUP
  LAND-SH-POSTMORTEM strong  PR-OPEN   audit/land-sh-postmortem 168h23m ago
  LAUNCH-PLAN-SH strong  PR-OPEN   feat/launch-plan-sh    123h22m ago
  ...
  MEMORY-INDEX-COMPACTION strong  PR-OPEN   feat/memory-index-compaction 121h22m ago
  METER-DOC-RECONCILE economy PR-OPEN   feat/meter-doc-reconcile 107h24m ago
  ...
  ON-DEMAND-TOOL-AUDIT frontier PR-OPEN   audit/on-demand-tools  122h48m ago
  PRICE-REFRESHER strong  PR-OPEN   feat/price-refresher   113h17m ago
  PRICING-LIMITS-CHECK-SH strong  PR-OPEN   feat/pricing-limits-check-sh 123h08m ago
  PRODUCT-GRADES-STORE strong  PARKED    feat/product-grades-store unclaimable — see note:
  PROJECT-MEMBERSHIP-GATE economy PR-OPEN   feat/project-membership-gate 164h31m ago
  REACHABILITY-AUDIT-LAND strong  blocked   feat/reachability-audit-land needs REACHABILITY-GATE
  REACHABILITY-GATE strong  PR-OPEN   feat/reachability-gate 122h55m ago
  ...
  SEMGREP-CI-REQUIRED-CHECK strong  ready     feat/semgrep-ci-required-check -
  SESSION-END-PUSH-GATE strong  PR-OPEN   feat/session-end-push-gate 137h30m ago
  SHELLCHECK-BLOCKING economy ready     feat/shellcheck-blocking -
  SSOT-DRIFT-GATE strong  PR-OPEN   feat/ssot-drift-gate   122h48m ago
  STALE-CHECK-SH strong  PR-OPEN   feat/stale-check-sh    123h02m ago
  STRANDED-WORK-AUDIT strong  ready     feat/stranded-work-detect -
  SYNC-SCHEDULE economy PR-OPEN   feat/sync-schedule     52h45m ago
  TSV-APPEND-UNIFY frontier PR-OPEN   feat/tsv-append-unify  122h37m ago
  VULTURE-INVESTIGATE-RETIRE-INERT strong  blocked   feat/vulture-investigate-retire-inert needs GITLEAKS-ADOPT, BANDIT-ADOPT
  WEB-ROADMAP-GENERATOR standard PR-OPEN   feat/web-roadmap-generator 121h14m ago
  ...
  WORK-GATE-UNIVERSAL strong  PR-OPEN   feat/work-gate-universal 122h58m ago
  ...
  SUMMARY  droids:0   ready:10  claimed:0  PR-open:32  done:6  blocked:17  parked:8
...
== validate_board ==
  ...
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/forwarder.py <- FT-WIRE-QUOTA GATEWAY-LITELLM-ADOPT WIRE-BRAIN-INTO-GATEWAY
  ...
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/proxy_server.py <- CG-LAN-OPEN-UI GATEWAY-LITELLM-ADOPT
  ...
  WARN owns-path-missing: BANDIT-ADOPT owns 'fleet/checks/bandit.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: BANDIT-ADOPT owns '.github/workflows/bandit.yml' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: BANDIT-ADOPT owns 'fleet/tests/bandit-canary.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: BANDIT-ADOPT owns 'fleet/tests/fixtures/bandit-known-bad.py' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: GATEWAY-LITELLM-ADOPT owns 'src/charon/litellm_router.py' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: GATEWAY-LITELLM-ADOPT owns 'tests/test_litellm_router.py' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: GITLEAKS-ADOPT owns 'fleet/checks/gitleaks.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: GITLEAKS-ADOPT owns '.github/workflows/gitleaks.yml' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: GITLEAKS-ADOPT owns 'fleet/tests/gitleaks-canary.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: GITLEAKS-ADOPT owns 'fleet/tests/fixtures/gitleaks-known-bad.txt' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: SEMGREP-CI-REQUIRED-CHECK owns 'fleet/checks/semgrep.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SEMGREP-CI-REQUIRED-CHECK owns '.github/workflows/semgrep.yml' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SEMGREP-CI-REQUIRED-CHECK owns 'fleet/semgrep-rules/charon-policy.yml' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SEMGREP-CI-REQUIRED-CHECK owns 'fleet/tests/semgrep-canary.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SEMGREP-CI-REQUIRED-CHECK owns 'fleet/tests/fixtures/semgrep-known-bad.py' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SHELLCHECK-BLOCKING owns 'fleet/tests/shellcheck-blocking.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: VULTURE-INVESTIGATE-RETIRE-INERT owns 'fleet/checks/vulture.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: VULTURE-INVESTIGATE-RETIRE-INERT owns '.github/workflows/vulture.yml' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: VULTURE-INVESTIGATE-RETIRE-INERT owns 'fleet/tests/vulture-canary.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: VULTURE-INVESTIGATE-RETIRE-INERT owns 'fleet/tests/fixtures/vulture-known-dead.py' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: VULTURE-INVESTIGATE-RETIRE-INERT owns 'fleet/state/VULTURE-EVAL.md' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WCI-ADVISORY justified-disjoint-dep (ok): BANDIT-ADOPT -> SEMGREP-CI-REQUIRED-CHECK (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): FINAL-E2E-REVIEW -> MODEL-PREFLIGHT (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GATEWAY-GRADE-ORDER-MVP -> GATEWAY-LITELLM-ADOPT (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GH-SEAM-CHOKEPOINT -> GITHUB-LIMITS-HARDENING (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GITLEAKS-ADOPT -> SEMGREP-CI-REQUIRED-CHECK (marked real build/correctness prereq)
  ...
  WCI-ADVISORY justified-disjoint-dep (ok): VULTURE-INVESTIGATE-RETIRE-INERT -> GITLEAKS-ADOPT (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): VULTURE-INVESTIGATE-RETIRE-INERT -> BANDIT-ADOPT (marked real build/correctness prereq)
...
BANDIT-ADOPT.md  tier=strong  depends_on=SEMGREP-CI-REQUIRED-CHECK
...
GATEWAY-GRADE-ORDER-MVP.md  tier=strong  depends_on=GATEWAY-LITELLM-ADOPT
GATEWAY-LITELLM-ADOPT.md  tier=strong  depends_on=
...
GITLEAKS-ADOPT.md  tier=strong  depends_on=SEMGREP-CI-REQUIRED-CHECK
...
SEMGREP-CI-REQUIRED-CHECK.md  tier=strong  depends_on=
SESSION-END-PUSH-GATE.md  tier=strong  depends_on=
SHELLCHECK-BLOCKING.md  tier=economy  depends_on=
...
VULTURE-INVESTIGATE-RETIRE-INERT.md  tier=strong  depends_on=GITLEAKS-ADOPT, BANDIT-ADOPT
...
1. The anti-hand-roll doctrine drift was **mode-4 handoff decision-loss**; fixed by putting it in the LOADED rules verbatim (§0) + the negative-column anti-patterns — NOT by building a new gate.
2. Removing the **stdlib-only prohibition UNBLOCKED the LiteLLM adopt** — its prior UNRESOLVED verdict rested on AP-1.
3. The **money-code e2e/dogfood bar CAUGHT a real gap**: the litellm build's first pass was on a stale base missing `egress.py`, and its new outbound path had to be reconciled to enforce the egress allowlist (now does — fail-on-revert proven).
