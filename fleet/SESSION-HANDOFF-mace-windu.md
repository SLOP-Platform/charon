# Charon Fleet — Session Handoff (2026-07-21T13:46:23Z) — mace-windu
...
```
Read and fully follow /home/stack/charon-private/fleet/SESSION-HANDOFF-mace-windu.md — you are the fresh Charon fleet MANAGER, carry it out, then flip to fleet mode.
```
...
**Session:** mace-windu
**Generated:** 2026-07-21T13:46:23Z
**Product HEAD:** 7e16e4a — current with origin/master
**Rig HEAD:** 8cb2ba7 — current with origin/master

---
<!-- GENERATED-STATE v1 (do not hand-edit) generated=2026-07-21T13:46:25Z -->
...
origin-master rig = 8cb2ba74192a1b5bf87a633d7380246a48baf017  # Nnyan/charon-private

**Open PRs — product (SLOP-Platform/charon):**
- #183 docs(adr-0020): litellm metering bridge (Proposed) — gates GW-BRIDGE-2 [draft=true] head=docs/adr-litellm-metering-bridge (CI-unverified until checked)
- #182 feat(gateway): GW-BRIDGE-1 — silent-downgrade guard on the litellm.Router path [draft=true] head=feat/gw-bridge1-downgrade-rehost (CI-unverified until checked)
...
- rig /home/stack/charon-private [master]: 0 commit(s) ahead of origin/master, 7 file(s) uncommitted
...
```
rig master HEAD:    8cb2ba7
rig master subject: feat(rig): net-diff re-derive the substrate-first gate onto current master
...
8cb2ba7 feat(rig): net-diff re-derive the substrate-first gate onto current master
fd1ef10 fix(board): peak-pricing real-dep marker; move RANK-0 queue to tracked session-notes/
a0a89ec chore(fleet): mace-windu session state — RANK-0 queue, session issues, peak-pricing ticket
03f49be doctrine(class-level): §0 anchor — CLASS-LEVEL / BLAST-RADIUS on every finding
33f7a51 board(gateway-adopt): decompose GATEWAY-LITELLM-ADOPT into 4 bridges + cutover
...
### Manager's first actions (priority order)

> This handoff SUPERSEDES the stale 2026-07-13 mace-windu handoff (its MODEL-LIFECYCLE/preflight mandate long since landed; old content preserved in git history).

**RANK-0 — DO FIRST (operator-mandated, all three at ZERO). Full detail + evidence: `fleet/session-notes/NEXT-SESSION-RANK0.md`.**

1. **R0.1 — the single move that kills all 3 standing concerns (rules-bloat / unenforced-rules / hand-rolling).** Activate **COVERAGE-META-GATE** (ready; build `fleet/checks/rule-coverage.sh` + `fleet/state/RULE-REGISTRY.tsv` by porting mediastack `enforcement_coverage.py` — TOOL-FIRST, do NOT rebuild; both files ABSENT today = ticketed-never-built) **AND land the boarded tool-adapters** SEMGREP-CI-REQUIRED-CHECK → GITLEAKS-ADOPT → BANDIT-ADOPT → VULTURE. Also = blast-radius blocker #3 + fixes the gitleaks false-coverage security gap.
2. **R0.2 — GRADER-SECFIX-RECONCILE.** Kill the live `shell=True` command-injection on master (`fleet/benchmark/graders/real.py:54`); per §0 CLASS rule, class-scan for ALL `shell=True`/injection sites and fix the class.
3. **R0.3 — reconcile-merged sweep.** Close the **20 already-done-but-open** tickets (list: `fleet/session-notes/blocker-blast-radius-audit.md`). Start from a truthful board.

**Then (this session's open threads):**
4. **Merge/verify the 3 staged PRs:** product **#182** (GW-BRIDGE-1 downgrade guard — reviewed clean, LAND-WITH-FOLLOWUP), product **#183** (ADR-0020 metering bridge — must be Accepted before GW-BRIDGE-2 builds). Your merge (manager can't).
5. **Verify substrate-first-gate fires in real rig CI** (landed `8cb2ba7`; adds a CI step + pinned PyYAML) — I only verified it locally (gates-must-actually-run).
6. **Continue the gateway bridges:** GW-BRIDGE-2 (behind ADR-0020) → 3 → 4 → GW-CUTOVER-LIVE-WIRE (deletes the hand-rolled money-path). GATEWAY-GRADE-ORDER-MVP now depends on GW-CUTOVER.
7. **P1 — the drift class-fix:** KS24 (lens-drift) + KS29 (registry-primitive) + adopt a GitHub merge queue. This session's dominant friction was ONE class (declared-vs-reality drift); see `fleet/session-notes/SESSION-ISSUES-mace-windu.md` + `drift-tooling-audit.md`.
...
/home/stack/charon-private                                                                       8cb2ba7 [master]
...
2026-07-21T13:42:46Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T13:42:46Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T13:42:46Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T13:42:46Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T13:42:46Z	out	my-model	too-slow-failover	rc=124; budget=1800s; model streamed output but did not finish -- latency-is-a-failure-class, model-attributable
2026-07-21T13:42:46Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T13:42:46Z	out	my-model	infra-fault-failover	rc=3; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-21T13:42:46Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-21T13:42:46Z	out	my-model	error-failover	rc=1; non-limit, non-infra failure (genuine model-attributable result)
2026-07-21T13:42:46Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
```
### Git
```
...
```
  ...
  CHARON-FLEET STATUS @ 2026-07-21T13:46:55Z
  ...
  ADR0016-DEPLOY-PRICED-COMPLETENESS strong  PR-OPEN   feat/adr0016-priced-completeness-guard 130h56m ago
  API-DECOMPOSE-CYCLE-FIX economy PR-OPEN   feat/api-decompose-cycle-fix 125h27m ago
  ASSIGN-DISPATCH-PICK-FIX strong  PR-OPEN   feat/assign-dispatch-pick-fix 127h38m ago
  ...
  BENCH-PROVISIONAL-SCORING strong  PR-OPEN   feat/bench-provisional-scoring 127h20m ago
  CAPABILITY-ACTUALS-DEADREF-CLEANUP economy PR-OPEN   feat/capability-actuals-deadref-cleanup 127h12m ago
  CAPTURE-WIRING-TIMEOUT-FIX strong  PR-OPEN   feat/capture-wiring-timeout-fix 58h24m ago
  CG-LAN-OPEN-UI strong  PARKED    feat/cg-lan-open-ui    unclaimable — see note:
  CHARON-FLOWCHART strong  PR-OPEN   docs/charon-flowchart  126h19m ago
  ...
  DECOMPOSER-ROUTE-THROUGH-SWITCHBOARD strong  PR-OPEN   feat/decomposer-route-through-switchboard 127h28m ago
  DONE-SH-INTEGRITY-FIX strong  blocked   feat/done-sh-integrity-fix needs GITHUB-LIMITS-HARDENING, VERIFY-MERGED-REPO-AWARE
  DROID-LIFECYCLE-REAP strong  PR-OPEN   feat/droid-lifecycle-reap 128h10m ago
  ...
  FOREMAN-MULTI-TRIGGER economy PR-OPEN   feat/foreman-multi-trigger 127h32m ago
  FOREMAN-WIRE economy DONE      feat/foreman-wire      -
  FT-CATALOG-SEED economy PR-OPEN   feat/ft-catalog-seed   152h23m ago
  FT-LIMITS-GROQ-RECONCILE economy PR-OPEN   feat/ft-limits-groq-reconcile 59h56m ago
  ...
  GATEWAY-NONTOKEN-METERING strong  PR-OPEN   feat/gateway-nontoken-metering 130h14m ago
  ...
  GITHUB-LIMITS-HARDENING strong  PR-OPEN   feat/github-limits-hardening 128h15m ago
  GITLEAKS-ADOPT strong  blocked   feat/gitleaks-adopt    needs SEMGREP-CI-REQUIRED-CHECK
  GRACEFUL-DEGRADE frontier PR-OPEN   feat/graceful-degrade  119h07m ago
  ...
  GW-BRIDGE-1-DOWNGRADE-REHOST strong  ready     feat/gw-bridge1-downgrade-rehost -
  GW-BRIDGE-2-METERING-SPEND strong  blocked   feat/gw-bridge2-metering-spend needs GW-BRIDGE-1-DOWNGRADE-REHOST
  GW-BRIDGE-3-STREAMING-SSE strong  blocked   feat/gw-bridge3-streaming-sse needs GW-BRIDGE-1-DOWNGRADE-REHOST
  GW-BRIDGE-4-PARK-COOLDOWN strong  blocked   feat/gw-bridge4-park-cooldown needs GW-BRIDGE-1-DOWNGRADE-REHOST
  ...
  HANDOFF-ROOT-ARCHIVE economy PR-OPEN   feat/handoff-root-archive 113h49m ago
  INERT-INSTANCE-DETECT strong  blocked   feat/inert-instance-detect needs CAPABILITY-ACTUALS-DEADREF-CLEANUP
  LAND-SH-POSTMORTEM strong  PR-OPEN   audit/land-sh-postmortem 175h10m ago
  LAUNCH-PLAN-SH strong  PR-OPEN   feat/launch-plan-sh    130h09m ago
  ...
  MEMORY-INDEX-COMPACTION strong  PR-OPEN   feat/memory-index-compaction 128h09m ago
  METER-DOC-RECONCILE economy PR-OPEN   feat/meter-doc-reconcile 114h11m ago
  ...
  ON-DEMAND-TOOL-AUDIT frontier PR-OPEN   audit/on-demand-tools  129h34m ago
  PEAK-PRICING-AWARE strong  blocked   feat/peak-pricing-aware needs PRICING-LIMITS-CHECK-SH
  PRICE-REFRESHER strong  PR-OPEN   feat/price-refresher   120h03m ago
  PRICING-LIMITS-CHECK-SH strong  PR-OPEN   feat/pricing-limits-check-sh 129h54m ago
  PRODUCT-GRADES-STORE strong  PARKED    feat/product-grades-store unclaimable — see note:
  PROJECT-MEMBERSHIP-GATE economy PR-OPEN   feat/project-membership-gate 171h17m ago
  REACHABILITY-AUDIT-LAND strong  blocked   feat/reachability-audit-land needs REACHABILITY-GATE
  REACHABILITY-GATE strong  PR-OPEN   feat/reachability-gate 129h42m ago
  ...
  SESSION-END-PUSH-GATE strong  PR-OPEN   feat/session-end-push-gate 144h16m ago
  SHELLCHECK-BLOCKING economy ready     feat/shellcheck-blocking -
  SSOT-DRIFT-GATE strong  PR-OPEN   feat/ssot-drift-gate   129h34m ago
  STALE-CHECK-SH strong  PR-OPEN   feat/stale-check-sh    129h48m ago
  STRANDED-WORK-AUDIT strong  ready     feat/stranded-work-detect -
  SUBSTRATE-FIRST-GATE 4       ready     feat/substrate-first-gate-fresh -
  SYNC-SCHEDULE economy PR-OPEN   feat/sync-schedule     59h31m ago
  TSV-APPEND-UNIFY frontier PR-OPEN   feat/tsv-append-unify  129h23m ago
  VULTURE-INVESTIGATE-RETIRE-INERT strong  blocked   feat/vulture-investigate-retire-inert needs GITLEAKS-ADOPT, BANDIT-ADOPT
  WEB-ROADMAP-GENERATOR standard PR-OPEN   feat/web-roadmap-generator 128h00m ago
  ...
  WORK-GATE-UNIVERSAL strong  PR-OPEN   feat/work-gate-universal 129h44m ago
  ...
  #183  docs/adr-litellm-metering-bridge  [draft]
  #182  feat/gw-bridge1-downgrade-rehost  [draft]
  ...
  SUMMARY  droids:0   ready:11  claimed:0  PR-open:32  done:6  blocked:22  parked:9
...
== validate_board ==
  ...
  WARN owns-path-missing: GW-BRIDGE-1-DOWNGRADE-REHOST owns 'tests/test_gw_bridge1_downgrade.py' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WCI-ADVISORY justified-disjoint-dep (ok): GW-BRIDGE-2-METERING-SPEND -> GW-BRIDGE-1-DOWNGRADE-REHOST (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GW-BRIDGE-3-STREAMING-SSE -> GW-BRIDGE-1-DOWNGRADE-REHOST (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GW-BRIDGE-4-PARK-COOLDOWN -> GW-BRIDGE-1-DOWNGRADE-REHOST (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): GW-CUTOVER-LIVE-WIRE -> GW-BRIDGE-1-DOWNGRADE-REHOST (marked real build/correctness prereq)
  ...
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: SUBSTRATE-FIRST-GATE (difficulty=4, 3 owned surfaces) — decompose (fleet/decompose.sh SUBSTRATE-FIRST-GATE) or justify ('serial_justified: <reason>')
