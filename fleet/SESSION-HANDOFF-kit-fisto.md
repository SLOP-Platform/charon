...
3. **KSF → private repo `SLOP-Platform/keystone` (KS6)** — operator APPROVED private. GATED on `ksf gate` being green on KSF itself, which needs the **KS4 inert-code false-positive fix**. That fix was left RUNNING as a detached NW `opencode` job at session end (operator chose to let it finish through the `/quit`). **Check it FIRST, in this order:**
   - **Is it done?** The job writes `fleet/state/overnight/KS4-INERT-FIX-REPORT.md` and prints a `PACKET:` line when done. Report exists = finished. Live progress log (exists now): `/home/stack/charon-private/fleet/state/overnight/KS4-INERT-FIX.log`.
   - **Verify green-is-not-proof:** `cd /home/stack/code/keystone && python3 -m ksf.cli gate; echo $?` must exit **0** (no false positive on `ksf.cli.cmd_reconcile` / `cmd_gate`), AND a truly-unreferenced orphan fn must still make it exit non-zero. Read the report's three verification outputs — do not trust its SUCCESS line.
   - **Interim signal (observed while the job was still running):** it fixed the original false positives (`cmd_reconcile`/`cmd_gate` now count as reachable), but the gate then flagged 2 module functions — `ksf.modules.error_injection.is_enabled` and `ksf.modules.graph_adapter.check_reuse`. JUDGE each: genuine false positive (invoked via module dispatch → extend reachability to module-registry edges) vs **genuinely inert** KS3/KS4 code built-but-not-wired (→ a real finding to fix, not a gate bug). Do NOT let the fix over-loosen reachability to mask real inert code.
   - **Only then** create `SLOP-Platform/keystone` **private** (`gh repo create SLOP-Platform/keystone --private`), add remote, push `main`, wire `ksf-gate.yml` CI. Confirm leak_guard passes first (no `/home/stack` paths, names, emails, IPs).
