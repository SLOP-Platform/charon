...
> **STATUS: APPROVED by operator 2026-07-08. ROLLOUT PENDING** — not yet placed in any rig.
> Reviewed adversarially by 4 lenses (token-accounting, latency/weak-model, correctness/output-degradation)
> + an empirical measurement; source reports in `scratch/doctrine-review-*.md` + `scratch/doctrine-measurement.md`.
> Approved DEFAULTS for the §5 open decisions: residue floor **~10k** (lower when context headroom is tight);
> **model→tier mapping DEFERRED to the real-outcomes pivot grades** (interim = current scorecard tiers:
> frontier/strong = auto-delegate + gate; sonnet/weak = whitelist-only); weak-coordinator whitelist, hook
> thresholds, R-estimator, and reconciliation ownership = rig-owner defaults, tunable. Rollout targets
> (Charon fleet rig / SLOP `AGENTS.md` / both) to be confirmed at rollout time. Supersedes the v1 8-rule
> contract captured in memory `coordinator-token-economy-doctrine`.
