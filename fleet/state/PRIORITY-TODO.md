...
| 7 | `FLEET-STATUS-BOARD` + `MISSING-CLASS-DETECTORS` | registry + runner + BIDIRECTIONAL meta-check so a check that is deleted, renamed or silently stops shows as MISSING. Nine measured classes have NO detector at all (uncommitted tools, untracked reviews, absent SSOT keys, deploy drift, catalog rot, zombie daemons, name-pool burn, the scheduled jobs themselves, operator-action staleness) |
