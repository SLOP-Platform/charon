# Charon Fleet — Session Handoff (2026-07-23T01:40:37Z) — depa-billaba
...
```
Read and fully follow /home/stack/charon-private/fleet/SESSION-HANDOFF-depa-billaba.md — you are the fresh Charon fleet MANAGER, carry it out, then flip to fleet mode.
```
...
**Session:** depa-billaba
**Generated:** 2026-07-23T01:40:37Z
**Product HEAD:** 0ef54b3 — current with origin/master
**Rig HEAD:** a2bf357 — current with origin/master

---
<!-- GENERATED-STATE v1 (do not hand-edit) generated=2026-07-23T01:40:39Z -->
...
origin-master rig = a2bf35730e5837a2653abd68b3b1779d46178be8  # Nnyan/charon-private
...
```
rig master HEAD:    a2bf357
rig master subject: chore(state): land 2 straggler reference files (plo-koon P7 disposition) (#158)
...
a2bf357 chore(state): land 2 straggler reference files (plo-koon P7 disposition) (#158)
a700566 handoff(rank0): lead with WORKLOOP-INTEGRITY-STACK-SPIKE (R0.0) — subsumes board-trust/inert-wiring (#157)
657c5c0 board(R0-lead): WORKLOOP-INTEGRITY-STACK-SPIKE — adopt-first spike of the work-loop stack (ao first) (#156)
2ef2fe9 chore(board): retire 5 merged rig tickets — GITLEAKS/BANDIT/VULTURE/GRADER-SECFIX/GRADER-REAL (#154)
866a32e handoff(rank0): refresh — prior R0.1/R0.2 done; new R0 = board-self-trust then inert-wiring-enforcement (#155)
...
> Context: prev session landed the full RANK-0 R0.1+R0.2 (gitleaks/bandit adopted, vulture rejected-with-proof,
> grader hardened, real.py:54 shell=True killed), retired 5 merged rig tickets, cleaned all stranded droid
> work + straggler files (main checkout is now FF-current + clean), re-mapped the code graph, and RE-BASED
> the priority stack on the operator-escalated decay class. Read `NEXT-SESSION-RANK0.md` FIRST (leads with R0.0).
> PROCESS (standing): work runs in DETACHED off-Claude droids by default; primary chat only for
> fast/would-degrade; right-size the tier (NOT frontier by default). `[work-detached-first-token-lean]`

1. **R0.0 LEAD — WORKLOOP-INTEGRITY-STACK-SPIKE** (`fleet/board/WORKLOOP-INTEGRITY-STACK-SPIKE.md`, on master).
   Durable fix for the built-but-not-wired / stale-board / gate-decay class = an ADOPT-FIRST STACK. Spike
   **agent-orchestrator ("ao") FIRST** (make-or-break: does it drive our Gitea-primary + GitHub-mirror
   topology, or is it GitHub-coupled? point it at the local gateway), then Omnigent, Windmill, Archon → a
   per-tool adopt/reject verdict + EVAL-REGISTRY row + one integrated adoption plan for operator review.
   Evidence base: `fleet/state/WORKLOOP-INTEGRITY-RESEARCH.md` (deep-research, 3 picks verified real). Detached, `[tier: strong]`.
2. **DROID POOL STAYS PARKED** until the board is trustworthy. Do NOT free-claim with `fleet-droid.sh <tier>` —
   merged rig tickets don't auto-retire (a droid re-claimed already-merged BANDIT-ADOPT this session, and
   left an orphan submitted-marker that RED'd the board). PIN droids to named open tickets only.
3. **Board interim (stopgap)** — reconcile-merged sweep: retire the remaining stale done-but-open rig tickets
   + clear orphan `fleet/state/submitted/*` markers (root: `retire-done.sh` / `_lib.sh` hardcode the PRODUCT
   repo, never read a ticket's `repo:` field). Subsumed by R0.0 but keeps the board usable meanwhile.
...
/home/stack/charon-private                                       a2bf357 [master]
...
2026-07-23T01:37:08Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-23T01:37:08Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-23T01:37:08Z	out	my-model	infra-fault-failover	rc=1; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-23T01:37:08Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-23T01:37:08Z	out	my-model	too-slow-failover	rc=124; budget=1800s; model streamed output but did not finish -- latency-is-a-failure-class, model-attributable
2026-07-23T01:37:08Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-23T01:37:08Z	out	my-model	infra-fault-failover	rc=3; provider/local/infra symptom (5xx/reset/refused/deadline/db-lock/timeout/opaque) -- not a model verdict
2026-07-23T01:37:08Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
2026-07-23T01:37:08Z	out	my-model	error-failover	rc=1; non-limit, non-infra failure (genuine model-attributable result)
2026-07-23T01:37:09Z	out	my-model	ALL-EXHAUSTED	every model failed over; POOL TOO THIN -> consider adding providers
```
### Git
```
master
...
```
  ...
  CHARON-FLEET STATUS @ 2026-07-23T01:41:07Z

  DROIDS (live tabs)        TIER    UPTIME    WORKING-ON
  strong-434684             strong  01:33:40  (idle/claiming)

  BOARD
  ID     TIER    STATE     BRANCH                 HELD-BY / NOTE
  ADR0016-DEPLOY-PRICED-COMPLETENESS strong  PR-OPEN   feat/adr0016-priced-completeness-guard 166h51m ago
  API-DECOMPOSE-CYCLE-FIX economy PR-OPEN   feat/api-decompose-cycle-fix 161h21m ago
  ASSIGN-DISPATCH-PICK-FIX strong  PR-OPEN   feat/assign-dispatch-pick-fix 163h33m ago
  ...
  BENCH-PROVISIONAL-SCORING strong  PR-OPEN   feat/bench-provisional-scoring 163h14m ago
  CAPABILITY-ACTUALS-DEADREF-CLEANUP economy PR-OPEN   feat/capability-actuals-deadref-cleanup 163h06m ago
  CAPTURE-WIRING-TIMEOUT-FIX strong  PR-OPEN   feat/capture-wiring-timeout-fix 94h18m ago
  CG-LAN-OPEN-UI strong  PARKED    feat/cg-lan-open-ui    unclaimable — see note:
  CHARON-FLOWCHART strong  PR-OPEN   docs/charon-flowchart  162h13m ago
  CREATION-GATE-DECOMPOSE-WIRE strong  blocked   feat/creation-gate-decompose-wire needs PROJECT-MEMBERSHIP-GATE
  DECOMPOSER-ROUTE-THROUGH-SWITCHBOARD strong  PR-OPEN   feat/decomposer-route-through-switchboard 163h22m ago
  DONE-SH-INTEGRITY-FIX strong  blocked   feat/done-sh-integrity-fix needs GITHUB-LIMITS-HARDENING, VERIFY-MERGED-REPO-AWARE
  DROID-LIFECYCLE-REAP strong  PR-OPEN   feat/droid-lifecycle-reap 164h04m ago
  ...
  FOREMAN-MULTI-TRIGGER economy PR-OPEN   feat/foreman-multi-trigger 163h27m ago
  FOREMAN-WIRE economy DONE      feat/foreman-wire      -
  FT-CATALOG-SEED economy PR-OPEN   feat/ft-catalog-seed   188h17m ago
  FT-LIMITS-GROQ-RECONCILE economy PR-OPEN   feat/ft-limits-groq-reconcile 95h50m ago
  ...
  GATEWAY-NONTOKEN-METERING strong  PR-OPEN   feat/gateway-nontoken-metering 166h09m ago
  ...
  GITHUB-LIMITS-HARDENING strong  PR-OPEN   feat/github-limits-hardening 164h10m ago
  GRACEFUL-DEGRADE frontier PR-OPEN   feat/graceful-degrade  155h01m ago
  ...
  HANDOFF-ROOT-ARCHIVE economy PR-OPEN   feat/handoff-root-archive 149h44m ago
  ...
  LAND-SH-POSTMORTEM strong  PR-OPEN   audit/land-sh-postmortem 211h04m ago
  LAUNCH-PLAN-SH strong  PR-OPEN   feat/launch-plan-sh    166h03m ago
  ...
  MEMORY-INDEX-COMPACTION strong  PR-OPEN   feat/memory-index-compaction 164h03m ago
  METER-DOC-RECONCILE economy PR-OPEN   feat/meter-doc-reconcile 150h05m ago
  ...
  ON-DEMAND-TOOL-AUDIT frontier PR-OPEN   audit/on-demand-tools  165h28m ago
  PEAK-PRICING-AWARE strong  blocked   feat/peak-pricing-aware needs PRICING-LIMITS-CHECK-SH
  PRICE-REFRESHER strong  PR-OPEN   feat/price-refresher   155h57m ago
  PRICING-LIMITS-CHECK-SH strong  PR-OPEN   feat/pricing-limits-check-sh 165h49m ago
  PRODUCT-GRADES-STORE strong  PARKED    feat/product-grades-store unclaimable — see note:
  PROJECT-MEMBERSHIP-GATE economy PR-OPEN   feat/project-membership-gate 207h11m ago
  REACHABILITY-AUDIT-LAND strong  blocked   feat/reachability-audit-land needs REACHABILITY-GATE
  REACHABILITY-GATE strong  PR-OPEN   feat/reachability-gate 165h36m ago
  ...
  SESSION-END-PUSH-GATE strong  PR-OPEN   feat/session-end-push-gate 180h10m ago
  SHELLCHECK-BLOCKING economy ready     feat/shellcheck-blocking -
  SSOT-DRIFT-GATE strong  PR-OPEN   feat/ssot-drift-gate   165h29m ago
  STALE-CHECK-SH strong  PR-OPEN   feat/stale-check-sh    165h43m ago
  ...
  SYNC-SCHEDULE economy PR-OPEN   feat/sync-schedule     95h25m ago
  TSV-APPEND-UNIFY frontier PR-OPEN   feat/tsv-append-unify  165h17m ago
  WEB-ROADMAP-GENERATOR standard PR-OPEN   feat/web-roadmap-generator 163h54m ago
  ...
  WORK-GATE-UNIVERSAL strong  PR-OPEN   feat/work-gate-universal 165h39m ago
  WORK-ROUTING-TO-CHARON-ENGINE frontier PARKED    feat/work-routing-to-charon-engine unclaimable — see note:
  WORKLOOP-INTEGRITY-STACK-SPIKE strong  ready     feat/workloop-stack-spike -
  ...
  SUMMARY  droids:1   ready:13  claimed:0  PR-open:32  done:5  blocked:15  parked:9
