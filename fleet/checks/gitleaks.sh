...
if [ -n "${GITLEAKS_CONFIG:-}" ]; then
  ...
  cat > "$CONFIG" <<'TOML'
title = "charon-rig gitleaks — extends the maintained default ruleset"
[extend]
useDefault = true
TOML
fi
...
if [ "$mode" = "diff" ]; then
  ...
  # EXCLUDE fleet/tests/fixtures/ : those are INTENTIONALLY-planted canary fixtures (e.g.
  # gitleaks-known-bad.txt). Scanning them in --diff mode would make a PR that ADDS a fixture red
...
# Scan each target with the maintained ruleset. --exit-code 1 makes a finding exit 1 (blocks merge);
...
worst=0
for t in "${targets[@]}"; do
  gitleaks dir --config "$CONFIG" --no-banner --redact --exit-code 1 "$t"
  rc=$?
...
case "$worst" in
  ...
  1) exit 1 ;;
  *) echo "gitleaks.sh: FAIL — gitleaks errored (exit $worst); failing closed." >&2; exit 2 ;;
esac
