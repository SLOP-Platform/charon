...
## 8b. THE TWO RULES THE OPERATOR ENFORCES HARDEST

These outrank convenience, schedule, and your own plan. If you learn nothing else, learn these.

### **WE DO NOT IGNORE PRE-EXISTING PROBLEMS**
A red you did not cause is still yours the moment you see it. "Pre-existing", "not mine",
"unrelated" are **not dispositions** — they are how a defect survives a hundred sessions. Every red
gets **fixed, or ticketed with a concrete diagnosis**. Silent red rots forever.

Live proof, found on 2026-07-25: `validate_board.sh` was rc=1 on an owns-collision **all session**
while 8 PRs were landed around it. `validate_board` IS the rig merge gate, so it was blocking every
rig land — and **nothing surfaced it.** It was found only by accident while validating an unrelated
ticket.

### **WE DO NOT WORK AROUND PROBLEMS — WE FIX ROOT / CLASS**
A workaround is a defect you agreed to keep. When you find something:
1. **NAME the class** the finding is an instance of.
2. **AUTO-SCAN for every other instance** — unprompted, without being asked.
3. **Fix or ticket the CLASS** as one shared primitive parameterised over the whole set, and
   **mechanize the detection** so it cannot silently recur.

**Fixing only the instance while the class stands is ITSELF a defect to be flagged.** And the
class fix must include: *what should have told us, and why didn't it?* A defect that no mechanism
surfaces is two problems — the defect, and the blindness.

Corollaries you will be tempted to break:
- Never bump a timeout to make something pass — slowness is a failure class; root-cause it.
- Never widen a gate, add a skip, or reach for a bypass to get moving. Five `WORK_LEASE_BYPASS`
  uses in one day is what that looks like on the way down.
- Never hand the operator a workaround command as if it were the fix. Say plainly it is a
  stopgap and name the root fix.
