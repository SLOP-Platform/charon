...
## Provenance (anti-clobber — verify this matches the filename before trusting anything below)

**Session:** agen-kolar
**Generated:** 2026-07-25 (hand-written during the session, NOT via handoff.sh — see caveat below)
**Rig origin/master at close:** `ec34714`
**Preserved work branch:** `origin/save/agen-kolar-board` @ `cc83c6b`
**Droid fix branch:** `origin/fix/DROID-CLIENT-PREFLIGHT-PATH` @ `8f0a4e5`

⚠️ **Caveat on this file's own provenance:** it was written by hand rather than generated by
`fleet/handoff.sh`, so the machine-queried GENERATED-STATE block that normally guarantees the facts
were live-queried is ABSENT. Every SHA and count here was verified by execution at the time of
writing, but treat it as a hand-assertion, not machine truth — **re-verify anything you are about
to act on**, which is the standing rule this session violated three times (see the relay hazard below).

## session-bridge (live board at close)

Queried live via the bridge (`board(repo=charon)` → `ok: true`). The bridge is a REMOTE daemon on
`rocinante`, reached through an SSH tunnel (pid 312) mapping `/run/charon-bridge/charon.sock` →
`~/.charon/coordinator-charon.sock`. **`/tmp/charon-bridge.sock` does NOT exist and its absence is
NOT evidence the bridge is down** — a design pass concluded exactly that and was wrong.

```
NAME                      REPO    STATUS       TICKET  LAST_SEEN            NOTE
saesee-tiin (MANAGER)     charon  in-progress  -       2026-07-24T16:49:50  STALLED 45,484s;
                                                                            lease expired;
                                                                            auto-nudge fired x2;
                                                                            nothing reaps it
```

> Coordination: this stalled session is live evidence for `DROID-LIFECYCLE-REAP` — the bridge is
> already emitting the expired/escalated signal and **no consumer acts on it**. That ticket's only
> remaining dep is `TICKET-MAP-GATE`, which LANDED today (PR #267). Pick a NEW Jedi name; never reuse.
