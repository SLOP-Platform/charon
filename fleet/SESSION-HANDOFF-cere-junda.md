# Charon Fleet — Session Handoff (2026-07-15T04:38:13Z) — cere-junda
...
**Session:** cere-junda
**Generated:** 2026-07-15T04:38:13Z
**Product HEAD:** b7aa4c8 — current with origin/master
**Rig HEAD:** eac3840 — current with origin/master

---

## THE MISSION (unchanged)
Get Charon's model-testing/eval system FULLY WORKING (valid, cheap discrimination of which model/leg/cost/skill is trusted) — all 8 EVAL-* tickets, plus this session's **mechanization tickets** that make the discipline gates, not doctrine. Autonomous, max-parallel, no idle.

## FIRST ACTION (operator directive — do BEFORE anything else)
**Check on ALL currently-running sessions/tabs:**
1. **2 operator droid TABS** (survive session end): were building `FT-QUOTA-ENGINE` + `FT-CONFIG-SURFACE` (base=master, product repo). Check `gh pr list` on both repos; review/land or let ride. NOTE: FT is DEFERRED behind eval — don't prioritize it.
2. **2 rebuild SUB-SESSIONS — COMMITTED (both done, fail-on-revert verified); FIRST TASK = adversarial-review each (BLOCKER/money-path) then `land.sh`:**
   - TAXONOMY: `feat/eval-taxonomy-align2` @ `f83810b` — chose the product-router 6 semantic classes (`matrix.py:20-27`), wrote `fleet/state/EVAL-TAXONOMY.md` (SSOT), repointed `grades.py` to `CANONICAL_WORK_CLASSES` + legacy map. Selftest 30+ green. PR #56 closed as superseded. Landing it unblocks EVAL-DERIVED-BUDGETS/TIER-CANON/SSOT-DRIFT-GATE.
   - GRADER: `feat/eval-grader-provision2` @ `38af193` — fixed the real F2 bug (`grader-daemon.py _snapshot_worktree` excludes `.hypothesis` / skip-logs unreadable cache), wrote `GRADER-PROVISION-NOTE.md`, control test verified. PR #57 closed. Landing unblocks all Wave 3.

## DELIVERED THIS SESSION
- 🟢 **Fixed the WRONG-BASE root cause**: origin/master was 17 commits stale behind unpushed `feat/live-lane-finalizer`; consolidated it → master (**PR #58**, `b466177`). Master now carries the eval architecture — the true base.
- 🟢 **Merged to master**: EVAL-LATENCY-GATE (#59, money-path, F4 fail-on-revert verified), LEG-PREFLIGHT-CANARY (#60, sandbox review LAND).
- 🟢 **Doctrine §12 committed** (mechanism-over-recall: ladder/break-even, docs-first, board-reuse-first, no-dismiss-preexisting, decompose-sizing-on-batches, §11 teeth, base-integrity).
- 🟡 **BUILT, needs land**: SESSION-CTX-PROPAGATE (`feat/session-ctx-propagate` `02219e6` + `fleet/state/SESSION-CTX-HOOK.json` to apply to `~/.claude/settings.json`); REVIEWER-DOGFOOD-REDS (`feat/reviewer-dogfood-reds` `cf53c81`).
- 🔵 **New tickets** (session recommendations, on board): SESSION-CTX-PROPAGATE, ENV-REGISTRY-WIRE, SSOT-DRIFT-GATE, WORK-GATE-UNIVERSAL, LEG-SANDBOX-HARDEN, LEG-F6-REALPATH-TEST, BASE-INTEGRITY-GATE, COVERAGE-META-GATE, STRANDED-WORK-AUDIT. Retired MEM-FINISH-WIRE (dup of FN1-5).
- 🔵 **Audit files** (read, don't re-derive): `fleet/state/STRANDED-WORK-AUDIT.md` (18 clean lands + FN #49-53), `SESSION-RECALL-CHALLENGE.md`, `MSOT-BLAST-RADIUS-AUDIT.md`.

## PENDING QUEUE (operator directive: consolidate → optimized waves → PUSH → keep driving EVAL)
1. Land TAXONOMY + GRADER rebuilds → unblocks EVAL Wave-2 (DERIVED-BUDGETS, TIER-CANON) + Wave-3.
2. Land SESSION-CTX-PROPAGATE; apply + verify `SESSION-CTX-HOOK.json` (a fresh sub-agent must receive the preamble).
3. Land REVIEWER-DOGFOOD-REDS.
4. **Recover stranded work** (operator pre-approved parallel push): land the 18 Tier-1 items in `STRANDED-WORK-AUDIT.md`; review+land FN #49-53 (memory epic, done-in-rig). Reconcile board-vs-GitHub drift (board "submitted"/PR-OPEN often ≠ reality).
5. **Consolidate ALL session mechanization tickets → optimized D&S waves (run `decompose_sizing` on big ones) → push**; build BASE-INTEGRITY-GATE (operator explicit), COVERAGE-META-GATE (port `mediastack/tools/enforcement_coverage.py`), WORK-GATE-UNIVERSAL, SSOT-DRIFT-GATE, ENV-REGISTRY-WIRE.
6. EVAL Wave-2 → Wave-3 (PIPELINE-CONSOLIDATE = **use `decompose_sizing`, cap 4**; then PROMOTION-GATE).

## OPERATOR ACTIONS
- **U (DE-RISKED):** the GRADER agent live-verified this box's `bench-grader` already has pytest and already DISCRIMINATES (tested via the live spool, no sudo) — the F2 blocker is fixed in code. `fleet/state/GRADER-PROVISION-NOTE.md` now holds ACL/deploy-hardening sudo steps (nice-to-have), NOT a Wave-3 blocker. Run when convenient.
- **AUTONOMOUS lever is ON** (`state/AUTONOMOUS`) — manager pushes/lands without asking; turn off via `fleet/autonomous.sh off`.

## GOTCHAS / LESSONS
- **WRONG-BASE**: never hoard integration on an unpushed branch (origin/master goes stale → tabs build base=master invalid). Verify a ticket's base contains its prereqs. BASE-INTEGRITY-GATE mechanizes this — build it.
- `decompose_sizing.py` (N*, cap 4, diminishing-returns 5%) EXISTS + is wired into `fleet/decompose.sh` — USE it on big batches; stop treating validate_board SPLITTABLE-SERIAL as advisory.
- **price-refresher is FABRICATED despite green CI** (stranded audit) — do NOT land it.
- 2 public-clean leaks flagged (trivial redacts) + 2 genuinely-incomplete builds — see STRANDED-WORK-AUDIT.md blockers.
- §11 teeth: port `mediastack/tools/enforcement_coverage.py` — a coverage-meta-gate would auto-catch advisory GAPs the operator kept catching by hand.

---

## Auto-generated state (from `handoff.sh` run at 2026-07-15T04:38:13Z)
...
```
  ...
  CHARON-FLEET STATUS @ 2026-07-15T04:38:26Z

  DROIDS (live tabs)        TIER    UPTIME    WORKING-ON
  strong-2056126            strong  41:12     FT-QUOTA-ENGINE
  strong-2056969            strong  40:55     FT-CONFIG-SURFACE

  BOARD
  ID     TIER    STATE     BRANCH                 HELD-BY / NOTE
  A1-LAND-GATE strong  PR-OPEN   feat/a1-land-gate      54h54m ago
  ADD-PROVIDER-MECHANIZE strong  PR-OPEN   feat/add-provider-mechanize 23h37m ago
  B3-LOG-PRUNE economy ready     chore/b3-log-prune     -
  B4-BRANCH-REAPER economy PR-OPEN   chore/b4-branch-worktree-reaper 54h58m ago
  BASE-INTEGRITY-GATE strong  ready     feat/base-integrity-gate -
  ...
  ENV-REGISTRY-WIRE strong  blocked   feat/env-registry-wire needs SESSION-CTX-PROPAGATE
  EVAL-DERIVED-BUDGETS strong  blocked   feat/eval-derived-budgets needs EVAL-TAXONOMY-ALIGN, LEG-PREFLIGHT-CANARY
  EVAL-GRADER-PROVISION strong  PR-OPEN   feat/eval-grader-provision 28m ago
  EVAL-LATENCY-GATE strong  DONE      feat/eval-latency-gate -
  EVAL-PIPELINE-CONSOLIDATE frontier blocked   feat/eval-pipeline-consolidate needs EVAL-TAXONOMY-ALIGN, EVAL-GRADER-PROVISION, EVAL-DERIVED-BUDGETS, EVAL-LATENCY-GATE, LEG-PREFLIGHT-CANARY
  EVAL-PROMOTION-GATE strong  blocked   feat/eval-promotion-gate needs EVAL-TAXONOMY-ALIGN, EVAL-GRADER-PROVISION, EVAL-PIPELINE-CONSOLIDATE
  EVAL-TAXONOMY-ALIGN strong  PR-OPEN   feat/eval-taxonomy-align 32m ago
  EVAL-TIER-CANON strong  blocked   feat/eval-tier-canon   needs EVAL-TAXONOMY-ALIGN
  ...
  FN1-MEMORY-STORE-ADOPT economy PR-OPEN   feat/fn1-memory-store  21h25m ago
  FN2-BITEMPORAL-DECAY economy PR-OPEN   feat/fn2-bitemporal-decay 21h02m ago
  FN3-CURATION-PASS economy PR-OPEN   feat/fn3-curation-pass 20h43m ago
  FN4-RESEARCH-GATE economy PR-OPEN   feat/fn4-research-gate 20h08m ago
  FN5-REGISTRY-SWEEP economy PR-OPEN   feat/fn5-registry-sweep 19h45m ago
  FORWARDER-RECONCILE strong  blocked   feat/forwarder-reconcile needs FAIL-LOUD-CONTRACT
  FT-CATALOG-SEED economy ready     feat/ft-catalog-seed   -
  FT-CONFIG-SURFACE strong  claimed   feat/ft-config-surface strong-2056969 · 32m
  FT-QUOTA-ENGINE strong  claimed   feat/ft-quota-engine   strong-2056126 · 28m
  FT-WIRE-QUOTA strong  blocked   feat/ft-wire-quota     needs FT-QUOTA-ENGINE, FT-CONFIG-SURFACE, FT-CATALOG-SEED, FAIL-LOUD-CONTRACT, FORWARDER-RECONCILE, PROVIDER-PROBE-FIX, PRICING-LIMITS-CHECKER
  GATE-INTEGRITY-A strong  PR-OPEN   feat/gate-integrity-inert 22h06m ago
  GATE-INTEGRITY-B strong  blocked   feat/gate-integrity-coverage needs GATE-INTEGRITY-A
  GATE-PERF strong  PR-OPEN   feat/gate-perf-product 23h08m ago
  ...
  HANDOFF-MECHANIZE economy PR-OPEN   feat/handoff-mechanize 19h07m ago
  HANDOFF-PIPEFAIL economy blocked   feat/handoff-pipefail  needs HANDOFF-MECHANIZE
  LAND-SH-POSTMORTEM strong  PR-OPEN   audit/land-sh-postmortem 22h01m ago
  LAND-SH-SAFE-SYNC strong  PR-OPEN   fix/land-sh-safe-sync  21h57m ago
  LAUNCH-PLAN-GATE strong  ready     feat/launch-plan-gate  -
  LEG-F6-REALPATH-TEST strong  ready     feat/leg-f6-realpath-test -
  LEG-PREFLIGHT-CANARY strong  DONE      feat/leg-preflight-canary -
  LEG-SANDBOX-HARDEN strong  ready     feat/leg-sandbox-harden -
  MODEL-PREFLIGHT frontier blocked   feat/model-preflight   needs BENCH-OOB-GRADING
  NO-DARK-WORK economy PR-OPEN   feat/no-dark-work      18h52m ago
  ...
  PROJECT-MEMBERSHIP-GATE economy PR-OPEN   feat/project-membership-gate 18h09m ago
  PROVIDER-CATALOG-REFRESH frontier ready     feat/provider-catalog-refresh -
  PROVIDER-PROBE-FIX strong  PR-OPEN   fix/provider-probe-validation 21h38m ago
  PROVIDER-URL-HELPER strong  blocked   refactor/provider-url-helper needs PROVIDER-PROBE-FIX
  R43-WIRING-AUDIT strong  PR-OPEN   audit/r43-wiring-audit 51h06m ago
  ...
  SESSION-CTX-PROPAGATE strong  claimed   feat/session-ctx-propagate mgr-subsession · 37m
  SR-4   economy PR-OPEN   feat/sr-4-smart-routing-doc-fix 17h46m ago
  SSOT-DRIFT-GATE strong  blocked   feat/ssot-drift-gate   needs EVAL-TAXONOMY-ALIGN
  ...
  TOOL-REPAIR-MUTATING economy PR-OPEN   feat/tool-repair-mutating-gate 17h37m ago
  ...
  WORK-CONVERGE-REVIEW frontier PR-OPEN   docs/work-converge-review 52h13m ago
  WORK-GATE-UNIVERSAL strong  ready     feat/work-gate-universal -
  ...
  SUMMARY  droids:2   ready:19  claimed:3  PR-open:22  done:3  blocked:21
...
== validate_board ==
  ...
  INFO owns hand-off (dep-sequenced/historical, ok): fleet/benchmark/dogfood-eval.sh <- EVAL-LATENCY-GATE EVAL-PIPELINE-CONSOLIDATE
  INFO owns hand-off (dep-sequenced/historical, ok): fleet/capability/grades.py <- EVAL-PROMOTION-GATE EVAL-TAXONOMY-ALIGN
  INFO owns hand-off (dep-sequenced/historical, ok): fleet/tests/leg-preflight.test.sh <- LEG-F6-REALPATH-TEST LEG-PREFLIGHT-CANARY
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/config.py <- DELETE-STATIC-RANK PROVIDER-URL-HELPER
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/forwarder.py <- FAIL-LOUD-CONTRACT FORWARDER-RECONCILE FT-WIRE-QUOTA
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/gate_runner.py <- GATE-INTEGRITY-B WIRE-MOCKLINT-ENFORCE
  INFO owns hand-off (dep-sequenced/historical, ok): src/charon/gateway.py <- FT-WIRE-QUOTA PRICING-LIMITS-CHECKER PROVIDER-PROBE-FIX
  ...
  WARN owns-path-missing: BASE-INTEGRITY-GATE owns 'fleet/checks/base-integrity.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: BASE-INTEGRITY-GATE owns 'fleet/tests/base-integrity.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: ENV-REGISTRY-WIRE owns 'fleet/env-registry.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: ENV-REGISTRY-WIRE owns 'fleet/tests/env-registry.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-DERIVED-BUDGETS owns 'fleet/benchmark/budget-derive.py' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-DERIVED-BUDGETS owns 'fleet/tests/budget-derive.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-GRADER-PROVISION owns 'fleet/benchmark/selftest/test_grader_env.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-GRADER-PROVISION owns 'fleet/state/GRADER-PROVISION-NOTE.md' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-PIPELINE-CONSOLIDATE owns 'fleet/benchmark/item-bank/' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-PIPELINE-CONSOLIDATE owns 'fleet/state/EVAL-PIPELINE-DESIGN.md' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-PROMOTION-GATE owns 'fleet/tests/promotion-gate.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-TAXONOMY-ALIGN owns 'fleet/state/EVAL-TAXONOMY.md' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: EVAL-TIER-CANON owns 'fleet/state/TIER-CANON.md' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: FT-CONFIG-SURFACE owns 'tests/test_config_free_tier.py' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: LEG-SANDBOX-HARDEN owns 'fleet/leg-preflight.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: LEG-SANDBOX-HARDEN owns 'fleet/tests/leg-sandbox-isolation.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WARN owns-path-missing: SESSION-CTX-PROPAGATE owns 'fleet/session-ctx-preamble.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SESSION-CTX-PROPAGATE owns 'fleet/tests/session-ctx-preamble.test.sh' does not exist (yet) — verify it is a to-be-created file or a typo
  WARN owns-path-missing: SESSION-CTX-PROPAGATE owns 'fleet/state/SESSION-CTX-HOOK.json' does not exist (yet) — verify it is a to-be-created file or a typo
  ...
  WCI-ADVISORY justified-disjoint-dep (ok): ENV-REGISTRY-WIRE -> SESSION-CTX-PROPAGATE (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-DERIVED-BUDGETS -> EVAL-TAXONOMY-ALIGN (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-DERIVED-BUDGETS -> LEG-PREFLIGHT-CANARY (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PIPELINE-CONSOLIDATE -> EVAL-TAXONOMY-ALIGN (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PIPELINE-CONSOLIDATE -> EVAL-GRADER-PROVISION (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PIPELINE-CONSOLIDATE -> EVAL-DERIVED-BUDGETS (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PIPELINE-CONSOLIDATE -> LEG-PREFLIGHT-CANARY (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PROMOTION-GATE -> EVAL-GRADER-PROVISION (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-PROMOTION-GATE -> EVAL-PIPELINE-CONSOLIDATE (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): EVAL-TIER-CANON -> EVAL-TAXONOMY-ALIGN (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): FINAL-E2E-REVIEW -> MODEL-PREFLIGHT (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): FT-WIRE-QUOTA -> FT-QUOTA-ENGINE (marked real build/correctness prereq)
  WCI-ADVISORY justified-disjoint-dep (ok): FT-WIRE-QUOTA -> FT-CONFIG-SURFACE (marked real build/correctness prereq)
  ...
  WCI-ADVISORY justified-disjoint-dep (ok): LEG-SANDBOX-HARDEN -> LEG-PREFLIGHT-CANARY (marked real build/correctness prereq)
  ...
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: BASE-INTEGRITY-GATE (difficulty=3, 2 owned surfaces) — decompose (fleet/decompose.sh BASE-INTEGRITY-GATE) or justify ('serial_justified: <reason>')
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: COVERAGE-META-GATE (difficulty=3, 3 owned surfaces) — decompose (fleet/decompose.sh COVERAGE-META-GATE) or justify ('serial_justified: <reason>')
  ...
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: LAUNCH-PLAN-GATE (difficulty=3, 3 owned surfaces) — decompose (fleet/decompose.sh LAUNCH-PLAN-GATE) or justify ('serial_justified: <reason>')
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: LEG-SANDBOX-HARDEN (difficulty=3, 2 owned surfaces) — decompose (fleet/decompose.sh LEG-SANDBOX-HARDEN) or justify ('serial_justified: <reason>')
  ...
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: SESSION-CTX-PROPAGATE (difficulty=3, 3 owned surfaces) — decompose (fleet/decompose.sh SESSION-CTX-PROPAGATE) or justify ('serial_justified: <reason>')
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: SSOT-DRIFT-GATE (difficulty=4, 3 owned surfaces) — decompose (fleet/decompose.sh SSOT-DRIFT-GATE) or justify ('serial_justified: <reason>')
  ...
  WCI-ADVISORY parallelizability: SPLITTABLE-SERIAL: WORK-GATE-UNIVERSAL (difficulty=4, 3 owned surfaces) — decompose (fleet/decompose.sh WORK-GATE-UNIVERSAL) or justify ('serial_justified: <reason>')
...
BASE-INTEGRITY-GATE.md  tier=strong  depends_on=
...
EVAL-DERIVED-BUDGETS.md  tier=strong  depends_on=EVAL-TAXONOMY-ALIGN, LEG-PREFLIGHT-CANARY
EVAL-GRADER-PROVISION.md  tier=strong  depends_on=
EVAL-LATENCY-GATE.md  tier=strong  depends_on=
EVAL-PIPELINE-CONSOLIDATE.md  tier=frontier  depends_on=EVAL-TAXONOMY-ALIGN, EVAL-GRADER-PROVISION, EVAL-DERIVED-BUDGETS, EVAL-LATENCY-GATE, LEG-PREFLIGHT-CANARY
EVAL-PROMOTION-GATE.md  tier=strong  depends_on=EVAL-TAXONOMY-ALIGN, EVAL-GRADER-PROVISION, EVAL-PIPELINE-CONSOLIDATE
EVAL-TAXONOMY-ALIGN.md  tier=strong  depends_on=
EVAL-TIER-CANON.md  tier=strong  depends_on=EVAL-TAXONOMY-ALIGN
...
FT-CONFIG-SURFACE.md  tier=strong  depends_on=
FT-QUOTA-ENGINE.md  tier=strong  depends_on=
FT-WIRE-QUOTA.md  tier=strong  depends_on=FT-QUOTA-ENGINE, FT-CONFIG-SURFACE, FT-CATALOG-SEED, FAIL-LOUD-CONTRACT, FORWARDER-RECONCILE, PROVIDER-PROBE-FIX, PRICING-LIMITS-CHECKER
...
LAUNCH-PLAN-GATE.md  tier=strong  depends_on=
LEG-F6-REALPATH-TEST.md  tier=strong  depends_on=LEG-PREFLIGHT-CANARY
LEG-PREFLIGHT-CANARY.md  tier=strong  depends_on=
LEG-SANDBOX-HARDEN.md  tier=strong  depends_on=LEG-PREFLIGHT-CANARY
...
SESSION-CTX-PROPAGATE.md  tier=strong  depends_on=
