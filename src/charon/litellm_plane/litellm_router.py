...
def _leg_model_name(agent_model: str, index: int) -> str:
    """Synthetic model_name for the index-th leg so a Charon chain maps to N
    distinct-named deployments chained via fallbacks (D-019)."""
...
def _deployment(
    ...
    """One model_list entry. model_name is the per-leg synthetic id. model_info
    carries the charon provider id for X-Charon header reconstruction."""
...
def build_fallbacks(chains_by_model: dict[str, list[UpstreamRoute]]) -> list[dict[str, list[str]]]:
    """Map chains to litellm fallbacks list (D-019). Multi-leg chains become
    {leg#0: [leg#1, ...]} entries; single-leg chains yield no entry."""
...
def _install_attempt_callbacks() -> None:
    """Install global litellm success/failure callbacks recording each leg's
    outcome into the per-request ATTEMPTS_META_KEY list. Idempotent."""
    import litellm
...
def _primary_leg(agent_model: str, chains: dict[str, list]) -> str | None:
    """The model_name litellm routes to first for agent_model."""
...
def _classify_for_envelope(provider: str, bt: Any) -> tuple[str, str]:
    """Source (class, rearm) for the envelope from the balance tracker."""
    if bt is None:
        return ("unknown", "unknown")
    fc = bt.funding_class(provider)
    if fc is None:
        return ("unknown", "unknown")
    _LABELS = {1: "free-recurring", 2: "flat-sub", 3: "drain-then-park", 4: "PAYG"}
    _REARM = {1: "auto reset (quota window)", 2: "operator top-up (next cycle)",
              3: "operator top-up", 4: "top-up or rate-limit cooldown"}
    try:
        return (_LABELS[int(fc)], _REARM[int(fc)])
...
def _synth_exhaustion_envelope(
    ...
    """Build the ADR-0016 terminal envelope. all_parked=True -> D-012 503;
    all_parked=False -> all_providers_exhausted 503."""
    if all_parked:
        legs = []
...
def complete_via_router_tracked(
    ...
    header reconstruction + D-012/D-018 envelope synthesis. Returns (status, dict, headers).
    D-012: fully-parked -> 503 all_legs_parked, zero upstream calls. On failure: 503
    all_providers_exhausted from recorded attempts. On success: 200 with headers."""
    ...
    try:
        ...
        return status, env, {"X-Charon-Failovers": str(failovers),
    ...
    headers: dict[str, str] = {"X-Charon-Failovers": str(len(failover_attempts))}
    if provider:
        headers["X-Charon-Provider"] = provider
    if failover_attempts:
        headers["X-Charon-Failover-Reasons"] = "; ".join(f"{r.provider}={r.status}" for r in failover_attempts)
