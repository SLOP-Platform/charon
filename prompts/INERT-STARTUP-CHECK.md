# SESSION — INERT-STARTUP-CHECK (P0 gate)

**Model:** NON-ANTHROPIC via the Charon gateway. Never Claude/Anthropic. Graded sample.
**Repo:** charon (PUBLIC) · **Ticket:** INERT-STARTUP-CHECK
**Worktree:** `/home/stack/charon-wt/INERT-STARTUP-CHECK` — ISOLATED. Do NOT work in `/home/stack/code/charon`
...
0. `NAME="$(bash /home/stack/charon-private/fleet/claim-jedi-name.sh)"; echo "claimed: $NAME"`
   Then `session-bridge_register(session_id="<NAME>", name="INERT-STARTUP-CHECK", repo="charon", ticket="INERT-STARTUP-CHECK",
...
2. `git -C /home/stack/code/charon worktree add -b $(grep '^branch:' /home/stack/charon-private/fleet/board/INERT-STARTUP-CHECK.md | cut -d' ' -f2) /home/stack/charon-wt/INERT-STARTUP-CHECK origin/master`
3. `cd /home/stack/charon-wt/INERT-STARTUP-CHECK`
4. **Read the ticket — it is BINDING and contains the full rationale, evidence and done-contract:**
   `/home/stack/charon-private/fleet/board/INERT-STARTUP-CHECK.md`
...
## LAST STEP (REQUIRED)
```
git add -A && git commit -m "INERT-STARTUP-CHECK: build the gate + red-proof its assertions"
```
