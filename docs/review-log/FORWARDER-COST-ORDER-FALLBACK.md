...
## RED then GREEN
4 tests FAIL on the named revert (empty meter → order unchanged):
...
All 7 GREEN with the fix. The 3 that pass on revert use the
live-meter path or the drain block, which are unaffected.
