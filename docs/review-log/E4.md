...
Decisions recorded BEFORE code; full rationale in `docs/adr/0018-intake-ticket-plan-phase1.md`.

**Change under review:** new `src/charon/intake.py` + `tests/test_intake.py` + ADR-0018.
...
- **[security] "Intake of untrusted text is an injection vector."** Resolved by
  construction (ADR-0018 D1): there is **no code path from input to execution**. Intake
...
- **[scope] "Owned-paths inference is the hard open question — don't over-build."**
  ...
  inference and no-path units are **flagged for the human**, not guessed (ADR-0018 D4). No
  static analysis.
...
- **[scope] "Don't touch sibling modules."** `decompose.py`/`land.py`/`validate.py` are
  ...
  `docs/adr/0018-*.md` (+ this fragment).
