...
- **NEW (small):** a thin `price_refresher.py` that ADOPTS best-in-class sources (vendor LiteLLM prices JSON + poll OpenRouter `/models` + changedetection.io webhooks — not a bespoke scraper); structured fail-loud `providers_tried`; delete static `cost_rank`.
